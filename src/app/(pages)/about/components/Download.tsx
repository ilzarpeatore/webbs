          ...
          <div className="order-1 flex flex-col items-center text-center md:me-10 md:text-left lg:items-start">
            ...
            <div className="relative z-40 mt-3.5 flex flex-col gap-4 lg:mt-8">
              {downloadButtonsData.map((btn, idx) => (
                <Link key={idx} href={btn.link} className={`group relative inline-flex overflow-hidden rounded-full px-3.5 transition-all duration-300 hover:scale-95 lg:px-8.5 ${btn.bgClass}`}>
                  <div className="relative flex flex-col items-center transition-transform duration-300 group-hover:-translate-y-full">
                    <div className="flex h-14 items-center gap-3">
                      <Image src={btn.icon} className="size-5" alt="" />
                      <span className="text-sm lg:text-base">{btn.label}</span>
                    </div>
